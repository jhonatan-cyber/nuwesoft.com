...
### Vue 3 & Laravel / Inertia.js Integration Rules

If the `PRODUCT.md` file (or context script output) indicates that the project uses Vue 3, Inertia.js, and/or Laravel, the agent MUST adhere to these strict integration rules:

* **Single File Components (.vue)**: Deliver all UI implementations and edits exclusively in SFC format using the `<script setup>` syntax and Vue 3 Composition API.
* **State & Reactivity**:
  - Use native reactivity (`ref`, `reactive`, `computed`) and Vue lifecycle hooks (`onMounted`, `onUnmounted`, `watch`).
  - Do NOT suggest React hooks (`useState`, `useEffect`, `useMemo`) or React state managers (Zustand, Redux). If global state is needed, use Pinia or standard Vue reactivity patterns (inject/provide).
* **Inertia.js Integration**:
  - Use Inertia's `<Link>` component instead of standard `<a>` or Vue Router's `<router-link>` for internal routing.
  - Handle backend data using Inertia page props (`$page.props` or `defineProps`).
  - Do NOT implement client-side routers or fetch data directly via Axios in components if it violates Inertia's server-driven data flow, unless explicitly requested.
* **Component Compilation & Builds**:
  - Do NOT try to run `npm run build` or similar compilation tasks. Just write correct, clean `.vue` code and let the project compile it in dev mode.
