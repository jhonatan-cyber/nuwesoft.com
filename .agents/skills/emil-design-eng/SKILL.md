...
## Vue 3 Animation & Transition Guide

When working on a Vue 3 project (as defined in `PRODUCT.md`), translate all React animation concepts to Vue's native tools and reactivity.

### 1. Entering & Exiting States: Native `<Transition>`
Instead of React's `AnimatePresence` or conditional rendering with state, use Vue's native `<Transition>` or `<TransitionGroup>`. This keeps animations clean, hardware-accelerated, and declarative.
...
### 2. Spring Physics in Vue 3 (using `@vueuse/motion`)
When dynamic physics are needed (e.g. Dynamic Island gestures or drag momentum), use `@vueuse/motion` (which runs off the same core logic as Framer Motion):
...
// Initialise spring physics on the element
useMotion(buttonRef, {
  initial: { scale: 1, y: 0 },
  hovered: { scale: 1.05 },
  pressed: { scale: 0.97 },
...
### 3. GSAP Integration in Vue SFC Lifecycle
When implementing advanced scroll timelines (like scroll-pinning, sticky-stack, or horizontal-pan), initialize GSAP inside `onMounted` and clean it up inside `onUnmounted` using `gsap.context()` to avoid memory leaks:
...
onMounted(() => {
  ctx = gsap.context(() => {
    const distance = trackRef.value.scrollWidth - window.innerWidth
...
onUnmounted(() => {
  if (ctx) ctx.revert() // Ensure cleanup
})
</script>

<template>
  <section ref="containerRef" class="relative overflow-hidden">
    <div ref="trackRef" class="flex h-[100dvh] items-center">
      <slot />
    </div>
  </section>
</template>
