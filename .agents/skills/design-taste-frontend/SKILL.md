...
|---|---|---|---|
| SaaS Landing (Linear-style) | 5 | 4 | 4 |
| Luxury / Premium DTC (Apple-y) | 7 | 6 | 3 |
| Creative Portfolio (Kinetic-type) | 8 | 8 | 2 |
| Corporate / Trust-First B2B | 3 | 2 | 5 |
| Experimental / Campaign Site | 10 | 9 | 4 |

---

## 3. PROJECT CONTEXT, VUE 3 SUPPORT & CONFLICT RESOLUTION

### 3.A Reading Project Context (PRODUCT.md)
* Before analyzing or writing code, the agent MUST look for a `PRODUCT.md` file in the root of the project.
* If `PRODUCT.md` exists, parse it to extract:
  - The project's technology stack (e.g., Laravel, Inertia.js, Vue 3, Tailwind v3/v4).
  - The target design aesthetic or brand rules.
* When the project is registered as a Vue 3 project, immediately deactivate React-specific recommendations, templates, and patterns. All layouts must be delivered in Vue Single File Components (`.vue` SFCs) using `<script setup>` and Composition API.

### 3.B Inter-Skill Hierarchy & Conflict Resolution
When multiple design-focused skills are active, resolve styling and structure decisions using this order of priority:
1. **`design-taste-frontend` (This Skill)**: Governs visual dials, aesthetic restraint, layout rhythm, color calibration, and anti-slop rules.
2. **`impeccable`**: Governs UX audit, micro-interaction state flows, form structures, and layout consistency. If `impeccable` defaults to React or Zustand but `PRODUCT.md` specifies Vue, the Vue rules from `design-taste-frontend` OVERRIDE `impeccable` defaults.
3. **`emil-design-eng`**: Governs animation details, spring physics parameters, and micro-motion polish. Animate only using Vue-compatible transitions (`<Transition>`, `<TransitionGroup>`, or vanilla CSS transitions/animations).

### 3.C Vue 3 Adaptation & Reactivity
* Use native Vue 3 features for responsive and dynamic states:
  - Reactivity: Use `ref()`, `reactive()`, and `computed()` instead of React's `useState` or `useMemo`.
  - Motion: Use Vue's `<Transition>` and `<TransitionGroup>` with custom CSS classes or Tailwind transition hooks.
  - Lifecycle: Use `onMounted` and `onUnmounted` for initializing and destroying third-party visual engines (like GSAP or canvas-based noise).
* Render templates using standard Tailwind utility classes or custom scoped CSS. Do not use React-specific libraries unless they have native Vue ports.

---

### 4.1 Fonts

* **FONTS:** Rotate from these modern, high-tier families (no generic system fonts):
